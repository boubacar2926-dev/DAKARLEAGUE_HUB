@php
    $statusFor = fn ($player) => $lineupsByPlayer->has($player->id)
        ? ($lineupsByPlayer[$player->id]->is_starting ? 'titulaire' : 'remplacant')
        : 'non_convoque';
@endphp
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.matches.lineup.update', $match) }}" class="space-y-6">
                                    ...
                                    @foreach ($team->players->sortBy('jersey_number') as $player)
                                        ...
                                        <div class="flex items-center justify-between text-sm border-b border-border pb-2 last:border-0">
                                            ...
                                            @if ($suspendedReason)
                                                ...
                                                <input type="hidden" name="{{ $side }}[{{ $player->id }}]" value="">
                                            @else
                                                <select name="{{ $side }}[{{ $player->id }}]" class="bg-background border-border text-white text-xs rounded-md focus:border-primary focus:ring-primary">
                                                    <option value="" @selected($statusFor($player) === 'non_convoque')>Non convoqué</option>
                                                    <option value="remplacant" @selected($statusFor($player) === 'remplacant')>Remplaçant</option>
                                                    <option value="titulaire" @selected($statusFor($player) === 'titulaire')>Titulaire</option>
                                                </select>
                                            @endif
                                        </div>
                                    @endforeach
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <x-primary-button>Enregistrer la composition</x-primary-button>
                </div>
            </form>
        </div>
    </div>
